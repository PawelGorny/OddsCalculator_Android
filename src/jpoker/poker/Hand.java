...
public class Hand /* implements Cloneable */
{
	 ...
	 * 
	 * Hand () Hand (int, int) Hand (int []) Hand (String)
	 */
	...
	/**
	 * The string which is passed in must contain standard character card encodings [2-9TJQK][cdhs] catenated with no spaces in between. All valid input strings
	 * will have an even number of characters.
	 */
	public Hand(String s) {
		this();

		/*
		 * StringTokenizer st = new StringTokenizer (s); while (st.hasMoreTokens()) { String str = st.nextToken(); for (int i=0; i<str.length(); i+=2) addCard
		 * (str.substring (i)); }
		 */
		String[] str = s.split(" ");
	...
	/**
	 * These functions doens't do any kind of bounds checking so it's use is DANGEROUS
	 */
	...
	/**
	 * This is an unsafe version of addHand, it does not check if the card already exists in the hand.
	 * 
	 * Normally this version should be avoided except in portions of the code where fast evaluation need to occur.
	 * 
	 * */
	 ...
	 * 
	 * 
	 * Old hand evaluation code. The evaluate1 function should always provide the correct answer to any evaluation.
	 * 
	 * The new code uses the HandEvaluator class for all evaluation. A hook is provided below for backwards compatability.
	 */

	/**
	 * there is an invariant that the cards in the hand must be consecutivly correct [0,52) starting with _cards[0],
	 * 
	 * The value returned is an integer value which uniuqely determines the value of the hand, such that there is a total ordering on all hands.
	 */
	public int evaluate() {
	...
	/*
	 * In order to comile the JNI part of this code:
	 * 
	 * 1) recompile this file: % javac poker/Hand.java 2) use javah on class: % javah poker.Hand 3) move to appropriate dir:
	 */
	/*
	 * public native void evaluateJNI (); static { System.loadLibrary("jni"); }
	 * 
	 * public int evaluateC () { // System.out.println ("evaluateC"); // _intval = 0; // if (_intval==0) { evaluateJNI (); // hashCards (); // if
	 * (evaluateFlush()) // return _intval; // if (evaluateStraight()) // return _intval; // evaluatePairing(); } return _intval; }
	 */

	public int evaluateLow(int n) {
		hashCards(); // probably wasting time here, oh well
		int hash = _card_hash[0] | _card_hash[1] | _card_hash[2] | _card_hash[3];
	...
	/**
	 * Used to generate code for a giant switch statement for evaluating straights. It wasn't very efficient in java.
	 */
	/*
	 * private static void makeStraightCode () { Debug.debug = true;
	 * 
	 ...
	 * 
	 * for (int i=0; i<8192; i++) { generateStraightValue (strval,i); }
	 * 
	 * for (int i=4; i<13; i++) { for (int j=0; j<8192; j++) { if (strval[j]==i) { System.out.println ("      case " + j + ":"); } } System.out.println
	 * ("        return " + i + ";"); }
	 * 
	 * System.out.println ("    }"); }
	 */
	/*
	 * private static void generateStraightValue (int ar[], int n) { int run=0; //int start=0; for (int i=Rank.NUM_RANK-1; i>=0; i--) { if ((n & 0x01<<i) > 0)
	 * run++; else run = 0; if (run == 5) ar[n] = i+4; }
	 * 
	 * if (run == 4 && (n & 0x01<<(Rank.NUM_RANK-1)) > 0) { ar[n] = 4; } }
	 */
	/*
	 * 
	 * TEST DRIVER
	...
	/*
	 * public static void setGood (boolean[] good, Hand [] h) { for (int i=0; i<h.length; i++) { //Debug.out (h[i].evaluate().toString()); if (h[i].evaluate() >
	 * NO_PAIR) good[i]=true; else good[i]=false; } }
	 * 
	 * 
	 * 
	 * public static void main (String args[]) throws Exception { Debug.debug = true;
	 * 
	 * Hand h = new Hand (); h.addCard ("9c"); h.addCard ("8d"); h.addCard ("9d"); h.addCard ("Qh"); h.addCard ("Jd"); h.addCard ("Tc"); h.addCard ("Kc");
	 * 
	 ...
	 * }
	 * 
	 * public static void main2 (String args[]) throws Exception { Debug.debug = true; int CARDS = 7; int OPEN = 4; int HANDS = 7; int PLAYERS = 8; int TRIALS =
	 * 500;
	 * 
	 * int c1 = (new Card ("As")).value(); int c2 = (new Card ("Kc")).value(); int c3 = (new Card ("9s")).value(); int c4 = (new Card ("8s")).value();
	 * 
	 * StudTable st = new StudTable (); st.getPlayer(5).fold(); st.deck.removeCard (c1); st.deck.removeCard (c2); // st.deck.removeCard (c3); //
	 * st.deck.removeCard (c4);
	 * 
	 * int winnah[] = new int [PLAYERS]; for (int i=0; i<TRIALS; i++) { Hand test = new Hand(); test.addCard (c1); test.addCard (c2); // test.addCard (c3); //
	 * test.addCard (c4);
	 * 
	 * st.newGame (); st.getPlayer(5).fold(); st.setHand (0,test); st.deck.shuffle (); st.dealToRiver (); st.determineWinningHand (); //st.print ();
	 * 
	 * winnah[st.getWinningPlayer()]++;
	 * 
	 * // if (TRIALS > 0) // { // Debug.out (st.getPlayer(0).hand + " " + st.getPlayer(0).hand.evaluate()); // Debug.out
	 * (st.getPlayer(st.getWinningPlayer()).hand // + " " + st.getPlayer(st.getWinningPlayer()).hand.evaluate()); // Debug.out ("\n"); // } }
	 * 
	 * for (int i=0; i<winnah.length; i++) { double pct = (double)winnah[i]/(double)TRIALS; double odds = 0; if (pct>0) odds = 1/pct - 1; Format.print
	 * (System.out, "player: %2d",i); Format.print (System.out, " : %5d", winnah[i]); Format.print (System.out, " : %5.2f\n", odds); } }
	 * 
	 * public static void main3 (String args[]) throws Exception { int counts[] = new int[10]; //for (int i=2; i<52; i++) for (int j=3; j<52; j++) for (int
	 * k=j+1; k<52; k++) for (int l=k+1; l<52; l++) for (int m=l+1; m<52; m++) { Hand h = new Hand (); h.addCard (0); h.addCard (1); h.addCard (2); h.addCard
	 * (j); h.addCard (k); h.addCard (l); h.addCard (m); //counts[h.evaluate().value()]++; //System.out.println (h.evaluate().value()); //System.out.println
	 * (h); }
	 * 
	 * System.out.println(""); int sum = 0; for (int i=0; i<9; i++) { Format.print (System.out, "%8d",counts[i]); sum += counts[i]; } System.out.println(" : " +
	 * sum); }
	 */
